...
export const create = (config) => {
  ...
  R.forEach((verb) => {
    sauce[verb] = R.partial(doRequest, [sauce, verb])
  })(VERBS)
...
/**
  Make the request.
 */
const doRequest = (api, method, url, params, data) => {
  const {axiosInstance} = api

  // convert our interface into something axios expects
  const axiosRequestConfig = {url, params, method, data}
