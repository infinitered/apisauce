...
export const create = (config) => {
  ...
  const doAsyncRequestTransforms = async (transforms, requestConfig) => {
    ...
    // await the promise pipeline (executing each asynchronous request
    // transform serially) and return the merged request config
    var transformedRequests = []
    return R.mergeAll([
      requestConfig,
      request,
      ...(
        await transforms.reduce(
          (curr, next) => {
            return curr.then(
              _ => next(request)
            ).then(
              transformed => transformedRequests.push(transformed)
            )
          },
          Promise.resolve()
        ).then(_ => transformedRequests.reverse())
      )
    ])
  }
